function [lambda,mu,u,x,z,LAMBDA,KKTError,costL,timeElapsed] = ...
    ...
    stepSizeZ     = ones(sizeSeg,DoP);
    stepSizeG     = ones(sizeSeg,DoP);
...
%     for i=1:1:DoP
        ...
        [lambda_i,mu_i,u_i,z_i,stepSizeMaxZ_i,stepSizeMaxG_i] = ...
            forward_correction_parallel_func(lambda_i,mu_i,u_i,x_i,z_i,p_i,dx_i,u_k_i,x_k_i,z_k_i,p_muu_F_i,LAMBDA_i,rho);
        
        lambda(:,:,i) = lambda_i;
        mu(:,:,i)     = mu_i;
        u(:,:,i)      = u_i;
        z(:,:,i)      = z_i;
        stepSizeZ(:,i) = stepSizeMaxZ_i;
        stepSizeG(:,i) = stepSizeMaxG_i;
    end
