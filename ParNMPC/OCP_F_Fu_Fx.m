function [F,Fu,Fx] = OCP_F_Fu_Fx(u,x,p,discretizationMethod,isMEnabled) %#codegen
    ...
    if isMEnabled
        [fdt,fudt,fxdt] = OCP_GEN_fdt_fudt_fxdt(u,x,p);
        % forced to 'Euler'
        M = OCP_GEN_M(u,x,p);
        invM = inv(M);
        invMf = invM*fdt;
        ...
        switch discretizationMethod
            case 'RK2'
                [fdt,fudt,fxdt] = OCP_GEN_fdt_fudt_fxdt(u,x,p);   
                [f12dt,fu12dt,fx12dt] = OCP_GEN_fdt_fudt_fxdt(u,x-fdt/2,p);   

                F  =  f12dt  - x;
                Fu =  fu12dt - 0.5*fx12dt*fudt;
                Fx =  fx12dt - 0.5*fx12dt*fxdt - Ix;
            case 'RK4'
                [fdt,fudt,fxdt]       = OCP_GEN_fdt_fudt_fxdt(u,x,p);   % k1
                [f12dt,fu12dt,fx12dt] = OCP_GEN_fdt_fudt_fxdt(u,x-fdt/2,p); % k2
                [f22dt,fu22dt,fx22dt] = OCP_GEN_fdt_fudt_fxdt(u,x-f12dt/2,p); % k3
                [f3dt,fu3dt,fx3dt]    = OCP_GEN_fdt_fudt_fxdt(u,x-f22dt,p); % k4
